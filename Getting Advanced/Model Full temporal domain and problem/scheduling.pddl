(define (domain scheduling)
	(:requirements :strips :typing :equality :negative-preconditions :fluents 
		...
		(:durative-action make-class
			:parameters(?t - teacher ?c1 ?c2 - class ?num1 ?num2 ?n1 ?n2 ?h1 ?h2 - numbers ?d1 ?d2 - date-week)
			...
			:effect(and
						...
						(at start (not(num-classes ?num1 ?c2 ?t))) ;; Ya se está realizando una de las clases
						...
						(at start (class-made ?c2 ?t ?d1)) ;; Clase realizado con un grupo un día de la semana ha sido realizada
						(at start (class-hours-made ?h1 ?h2 ?c2 ?d1)) ;; Clase realizada de un grupo en un intervalo de horas de un día
						...
						(at end (at-location ?c2 ?t)) ;; Durante todo el proceso ya se encuentra en la localización final
